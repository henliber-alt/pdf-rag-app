...
def summarize(payload: SummarizeRequest, db: Session = Depends(get_db)) -> dict:
    ...
    answer = answer_with_context(
        "סכם את המסמך בצורה מפורטת בעברית עם סעיפים עיקריים",
        [
            {
...
def compare(payload: CompareRequest, db: Session = Depends(get_db)) -> dict:
    ...
    answer = answer_with_context(
        "השווה בין המסמכים הבאים בעברית: דמיון, הבדלים, נושאים מרכזיים, מסקנות",
        contexts,
    )
...
@app.post("/api/diff")
def diff_docs(payload: DiffRequest, db: Session = Depends(get_db)) -> dict:
    ...
    raw_diff = "\n".join(
        unified_diff(old_text, new_text, fromfile="older", tofile="newer", n=2)
    )

    answer = answer_with_context(
        "סכם בעברית מה השתנה בין הגרסה הישנה לחדשה. התמקד בתוספות, מחיקות ושינויים מהותיים.",
        [
            {
                "document_name": "diff",
                "page_number": 1,
